 ...
 */
export const getPendingReschedules = async (userId: number, role: string): Promise<RescheduleRequest[]> => {
  let query = '';
 ...
 */
export const cancelRescheduleRequest = async (rescheduleId: number, userId: number): Promise<void> => {
  const reschedule = await getRescheduleById(rescheduleId);
